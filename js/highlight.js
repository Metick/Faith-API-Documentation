    ...
    return {
        ...
        keywords: {
            ...
            built_in: '_G _ENV _VERSION __index __newindex __mode __call __metatable __tostring __len ' +
                ...
                'tonumber tostring type unpack xpcall arg self ' +
                'coroutine resume yield status wrap create running ' +
                'sethook getmetatable gethook setlocal traceback setfenv getlocal getfenv ' +
                'io lines write close flush open output type read stderr stdin input stdout popen tmpfile ' +
                'math log max acos huge ldexp pi cos tanh pow deg tan cosh sinh  randomseed frexp ceil floor rad abs sqrt modf asin min mod fmod log10 atan2 exp sin atan ' +
                'os exit setlocale date getenv difftime remove time clock tmpname rename execute package preload loadlib loaded loaders config seeall ' +
                'sub upper gfind rep find match char dump gmatch reverse format gsub lower ' +
                'setn insert getn foreachi maxn foreach concat sort remove ' +
                'ModuleScript LocalScript ScriptSignal TouchTransmitter Instance ClickDetector ClickDetector Connection Color3 Vector2 BasePart [write-only]'
        },
    };
})